...
DECLARE_DELEGATE_RetVal_OneParam(FCropSheetData, FRequestCropSheetData, const FName&);
DECLARE_DELEGATE_RetVal_ThreeParams(AActor*, FRequestSpawnItemPickup, FString, const FVector&, const FRotator&);

DECLARE_DELEGATE_OneParam(FRequestUpdateCropData, AFarmlandTile*);
DECLARE_DELEGATE_OneParam(FRequestRemoveCropData, AFarmlandTile*);
/*
* Grid를 점유하고 작물을 키울 땅 타일
* 작물의 정보와 성장 정도에 따라 땅 Mesh 위에 작물 Mesh가 나타날 것
*/
UCLASS()
class GAMECONTENTS_API AFarmlandTile : public AActor, public IInteractive
{
	GENERATED_BODY()
